...
def _add_privmsg_to_view(
    ...
    *,
    ...
    if isinstance(view, views.ChannelView):
        ...
        if view.server_view.nick not in all_nicks:
            # Possible, if user is kicked
            all_nicks.append(view.server_view.nick)
    else:
        all_nicks = [view.nick_of_other_user, view.server_view.nick]

    parts = []
    for substring, base_tags in textwidget_tags.parse_text(text):
        for subsubstring, nick_tag in backend.find_nicks(
            substring, view.server_view.nick, all_nicks
        ):
            tags = base_tags.copy()
    ...
    if sender == view.server_view.nick:
    ...
    if isinstance(view, views.PMView):
        ...
        pinged = any(
            ...
            for substring, tag in backend.find_nicks(
                text, view.server_view.nick, [view.server_view.nick]
            )
        )
...
def _handle_privmsg(
    ...
    if recipient == server_view.nick:  # actual PM
...
def _handle_part(
    ...
    if parting_nick == server_view.nick:
...
def _handle_mode(
    ...
    if target_nick == channel_view.server_view.nick:
        target_tag = "self-nick"
    else:
        target_tag = "other-nick"

    if setter_nick == channel_view.server_view.nick:
...
def _handle_kick(server_view: views.ServerView, kicker: str, args: list[str]) -> None:
    ...
    if kicker == channel_view.server_view.nick:
        kicker_tag = "self-nick"
    else:
        kicker_tag = "other-nick"

    if kicked_nick == channel_view.server_view.nick:
        channel_view.add_message(
            [
...
def _handle_endofmotd(server_view: views.ServerView) -> None:
    server_view.core.send(f"WHOIS {server_view.nick}")
...
def _handle_literally_topic(
    ...
    if who_changed == channel_view.server_view.nick:
        nick_tag = "self-nick"
    else:
        nick_tag = "other-nick"
    ...
    # TODO: figure out what MODE with 2 or 4 args is
        ...
        for user_view in server_view.get_subviews(include_server=True):
            ...
            if isinstance(user_view, views.ChannelView):
                user_view.userlist.set_away(server_view.nick, False)
...
def handle_event(event: backend.IrcEvent, server_view: views.ServerView) -> None:
    match event:
        ...
        case backend.StateChanged(new_nick=nick, new_host=host):
            if server_view.nick != nick:
                server_view.nick = nick
                server_view.irc_widget.update_nick_button()
